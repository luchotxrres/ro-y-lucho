...
const App: React.FC = () => {
  ...
  const SectionIcon = ({ children, className = "" }: { children: React.ReactNode, className?: string }) => (
    <div className={`w-20 h-20 md:w-24 md:h-24 bg-wedding-white rounded-full flex items-center justify-center mb-10 mx-auto text-wedding-accent border border-wedding-neutral shadow-sm transition-transform hover:scale-110 duration-500 ${className}`}>
      {children}
    </div>
  );
    ...
    <div className="min-h-screen bg-wedding-bg text-wedding-text selection:bg-wedding-primary/20 selection:text-wedding-white">
      ...
      {!hasEntered ? (
        ...
        <>
          ...
          {/* GREETING - CON CORAZÓN ANIMADO PERSONALIZADO */}
          <Section className="text-center max-w-4xl mx-auto pt-40 pb-20 px-6">
            <SectionIcon className="heart-pulse overflow-hidden p-4">
              <svg 
          ...
          <Section className="text-center py-40 bg-wedding-white px-6">
            ...
            <h3 className="text-5xl font-serif text-wedding-primary italic mb-10">Mesa de Regalos</h3>
